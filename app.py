...
# Enhanced candidate registration form
def candidate_registration():
    ...
    with st.form("registration_form"):
        ...
        languages = st.multiselect(
            "Programming Languages",
            ["Python", "Java", "C#", "JavaScript", "Ruby", "PHP", "Go", "Swift", "Other"]
        )
        ...
        with cert_cols[1]:
            cert_provider = st.text_input("Certification Provider")
        with cert_cols[2]:
        ...
        with int_cols[2]:
            internship_cert = st.file_uploader("Upload Internship Certificate", type=["pdf", "jpg", "png"])
        ...
        if submitted:
            ...
            try:
                # Insert candidate
                cursor.execute("""
                    INSERT INTO candidates (name, email, degree, specialization, 
                    phone_number, linkedin_profile, github_profile)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                """, (name, email, degree, specialization, phone, linkedin, github))
                ...
                for lang in languages:
                    cursor.execute("""
                        INSERT INTO programming_languages (candidate_id, language_name)
                        VALUES (%s, %s)
                    """, (candidate_id, lang))
                
                # Insert certification
                if cert_name and cert_provider:
                    cert_file_path = None
                    if cert_file:
                        cert_file_path = f"certificates/{candidate_id}_{cert_file.name}"
                        save_uploaded_file(cert_file, cert_file_path)
                    
                    cursor.execute("""
                        INSERT INTO certifications (candidate_id, cert_name, cert_provider, cert_file_path)
                        VALUES (%s, %s, %s, %s)
                    """, (candidate_id, cert_name, cert_provider, cert_file_path))
                
                # Insert internship details
                if company_name and duration:
                    internship_file_path = None
                    if internship_cert:
                        internship_file_path = f"internships/{candidate_id}_{internship_cert.name}"
                        save_uploaded_file(internship_cert, internship_file_path)
                    
                    cursor.execute("""
                        INSERT INTO internships (candidate_id, company_name, duration, certificate_path)
                        VALUES (%s, %s, %s, %s)
                    """, (candidate_id, company_name, duration, internship_file_path))
...
def generate_reports():
    ...
    if report_type == "Individual Candidate":
        ...
        generate_college_scorecard(cursor)
    elif report_type == "Topper List":
        generate_topper_list(cursor)
    elif report_type == "Batch Comparison":
        generate_batch_comparison(cursor)
    elif report_type == "Feedback Analysis":
        generate_feedback_analysis(cursor)
